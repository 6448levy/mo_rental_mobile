...
class RatePlanController extends GetxController {
  ...
  void onInit() {
    super.onInit();
    debugPrint('📊 RatePlanController initialized');
  ...
  Future<void> loadRatePlans({int page = 1}) async {
    ...
    try {
      ...
      if (authToken != null) {
        ...
        debugPrint(
            '🔑 Token (first 30 chars): ${authToken!.substring(0, tokenLength)}...');
      }

      final request = RatePlanRequest(
        branchId: selectedBranch.value.isNotEmpty ? selectedBranch.value : null,
        vehicleClass: selectedVehicleClass.value.isNotEmpty
      ...
      debugPrint('📊 Plans count: ${response.data?.plans.length ?? 0}');
      debugPrint('📊 ======================================\n');
      ...
      if (response.success && response.data != null) {
        ...
        if (message.contains('Access denied') ||
          ...
          errorMessage.value =
              'Access denied. Only managers/admins can view rate plans.';
          Get.snackbar(
            'Access Denied',
  ...
  void applyFilters() {
    ...
    debugPrint('📍 Branch: ${selectedBranch.value}');
    debugPrint('🚗 Class: ${selectedVehicleClass.value}');
    debugPrint('💰 Currency: ${selectedCurrency.value}');
    debugPrint('✅ Active Only: ${showActiveOnly.value}');
    debugPrint('📅 Date: ${selectedDate.value}');
  ...
  Future<ApiResponse<RatePlan>> updatePlan(
